    ...
    public partial class ODEditor_MainForm : Form
    {
        ...
        private void exportCanOpenNodeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (tabControl1.SelectedTab != null)
            {
                ...
                sfd.FileName = (type == ExporterFactory.Exporter.CANOPENNODE_V4) ? "micro_canopen_od.h" : "micro_canopen_od.c";
        ...
        public  async Task<string> get_trans_strAsync(ODentry od)
        {
            ...
            while (retry >0)
            {
                ret = await  ts.TransAsync(od.parameter_name);
                if (ret.Length > 0)
                { 
                return ret;
                }
        ...
        // 异步更新 toolStripStatusLabel1
    
        async void Trans_thread()
        {
            int count = 0;
            int tot = 0;
            string ts_str = "";
            if (network.Count > 0)
            {
                ...
                EDSsharp eds = network[0];
                count = eds.ods.Count;

                foreach (ODentry od in eds.ods.Values)
                {
                    tot++;
                    toolStripStatusLabel1.Text = ((float)(count / tot)).ToString();

                    if (od.Nosubindexes == 0)
                    {
                        if (od.parameter_name_trans.Length == 0)
                        {
                            ts_str = await get_trans_strAsync(od);
                            od.parameter_name_trans = ts_str;
                        }
                    ...
                    {
                        foreach (ODentry od_obj in od.subobjects.Values)
                        {
                            if (od_obj.parameter_name_trans.Length == 0)
                            {
                                ts_str = await get_trans_strAsync(od_obj);
                                od_obj.parameter_name_trans = ts_str;
                            }
                        }
                    }
                }
        ...
        {
            // 开启一个线程
            Thread myThread = new Thread(new ThreadStart(Trans_thread));//实例
            myThread.Start();
